        ...
        HttpTransmitter.prototype.transmit = function (commands, onDone) {
            ...
            this.http.onerror = function (evt) {
                if (_this.errorHandler) {
                    ...
                    _this.errorHandler({ url: _this.url, httpStatus: _this.http.status });
                }
                onDone([]);
            };
                        ...
                        if (responseText.trim().length > 0) {
                            ...
                            if (_this.errorHandler) {
                                _this.errorHandler({
                                    ...
                                    exceptionClass: _this.http.getResponseHeader("X-OpenDolphin-Exception.Class"),
                                    exceptionMessage: _this.http.getResponseHeader("X-OpenDolphin-Exception.Message")
                                });
                            }
                        }
                    }
                    else {
                        console.log("** http.status NOT 200: ", evt);
                        if (_this.errorHandler) {
                            _this.errorHandler({
                                url: _this.url,
                                httpStatus: _this.http.status,

...
Note that you implement the server part with other frameworks like Grails for example but since this is
...
Since open-dolphin always keeps the PMs of the client and server in sync (so called shared presentation models) the change will be immediately visible
